#!/usr/bin/env bash
# Phase 2A.1 closed-loop smoke: Show-o2 acts in LIBERO for T steps.
set -eo pipefail
...
python3 "${MIRAGE_DIR}/scripts/phase2a_closed_loop.py" 2>&1 | tee "${LOG_DIR}/phase2a_closed_loop.log"
