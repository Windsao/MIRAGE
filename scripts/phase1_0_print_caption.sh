#!/usr/bin/env bash
# Phase 1.0 smoke wrapper for phase1_0_print_caption.py
# Caption gets printed to stdout (visible in this wrapper's log).
set -eo pipefail
...
export PYTHONPATH=${SHOWO2_PATH}:${PYTHONPATH:-}
...
python3 "${MIRAGE_DIR}/scripts/phase1_0_print_caption.py" 2>&1 | tee "${LOG_DIR}/phase1_0_caption.log"
